...
class MyFrame(tk.Frame):
    def __init__(self, root):
        ...
        self.b1 = tk.Button(self, text="下載YT(mp3)", command=self.analyse)
        self.b1.pack(expand=True, fill=tk.BOTH, pady=5)
        
        self.b2 = tk.Button(self, text="下載YT(mp4)", command=self.analyse2)
        self.b2.pack(expand=True, fill=tk.BOTH, pady=5)
    ...
    def analyse2(self):
        def work():
            ...
            path = r'.\music'
            yt_list = str_to_ytlink(text)
            download_list(yt_list, path, False)
            
            self.b2["state"] = tk.ACTIVE
        thread = threading.Thread(target=work)
        thread.start()
